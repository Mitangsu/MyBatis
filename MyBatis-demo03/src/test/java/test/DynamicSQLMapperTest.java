...
public class DynamicSQLMapperTest {
    ...
    /**
     * 动态SQL:
     * 1.if:根据标签中test属性所对应的表达式决定标签中的内容是否需要拼接SQL中
     * 2.where:当where标签中有内容时,会自动生成where关键字,并且将内容前多余的and或or去掉
     *         当where标签中没有内容时,此时where标签不会有任何效果
     ...
     *
     * 4.choose、when、otherwise,相当于switch
    ...
    public void testGetEmpByCondition(){
        SqlSession sqlSession = SqlSessionUtils.getSqlSession();
        DynamicSQLMapper mapper = sqlSession.getMapper(DynamicSQLMapper.class);
        List<Emp> list = mapper.getEmpByCondition(new Emp(null, "", 12, "男", "123@qq.com"));
